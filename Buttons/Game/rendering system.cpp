...
void RenderingSystem::render(Registry &registry, const glm::mat3 &viewProj) {
  ...
  CHECK_OPENGL_ERROR();*/
  const size_t indiciesSize = 3;//sizeof(ElemType) * QUAD_INDICIES * numQuads;
...
void RenderingSystem::fillIndicies(const size_t minQuads) {
  if (indicies.size() < minQuads * QUAD_INDICIES) {
    const size_t neededSize = minQuads - indicies.size() / QUAD_INDICIES;
...
void RenderingSystem::onLevelLoad(Registry &registry) {
  ...
  arrayBuf = GL::makeArrayBuffer(vertices, sizeof(vertices));
  /*GLuint vbo;
  glGenBuffers(1, &vbo);
  ...
  GL::Shader vertexShader = GL::makeShader(GL_VERTEX_SHADER, vertexSource, strlen(vertexSource));

  // Create and compile the fragment shader
  GL::Shader fragmentShader = GL::makeShader(GL_FRAGMENT_SHADER, fragmentSource, strlen(fragmentSource));

  // Link the vertex and fragment shader into a shader program
  program = GL::makeShaderProgram(vertexShader, fragmentShader);
  program.use();

  // Specify the layout of the vertex data
  GLint posAttrib = 0;
  GL::attribPointer<0, glm::vec2>(0, 0);
  GL::enable<0, glm::vec2>();
